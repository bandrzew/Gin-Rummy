...
public interface CardRepository extends JpaRepository<Card, Long> {
	...
	@Query("select c from Card c where c.cardHolder.name = ?1 and c.color = ?2 and c.isInMeld = false")
	List<Card> findByColorToRun(String cardHolderName, String color, Sort sort);
	...
	// @Modifying
	// @Query("delete from Cards c where c.id = ?1")
